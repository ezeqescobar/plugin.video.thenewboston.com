...
__addon_logo_icon__   = 'special://home/addons/%s/icon.png' % __addon_id__
...
def add_heading(title, linebreak=False, thumbnail=__addon_logo_icon__):
        if linebreak:
            li = xbmcgui.ListItem(thumbnailImage=thumbnail)
            li.setProperty("IsPlayable", "false")
            xbmcplugin.addDirectoryItem(__addon_id_int__, url="none", listitem=li, isFolder=False)

        li = xbmcgui.ListItem(label="[COLOR FF007EFF]" + title + "[/COLOR]", thumbnailImage=thumbnail)
        li.setProperty("IsPlayable", "false")
        xbmcplugin.addDirectoryItem(__addon_id_int__, url="none", listitem=li, isFolder=False)

def add_directory_link(title, mode,url=None, is_folder=True,
                       is_playable=False, total_items=0, thumbnail=__addon_logo_icon__):
